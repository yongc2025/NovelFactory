...
async def _check_rules(chapter, characters_info, foreshadows_status, chapter_draft) -> dict:
    ...
    response = await complete(messages=messages, role="editor_rules", temperature=0.1, max_tokens=2048)
...
async def _check_quality(chapter, chapter_draft, word_count) -> dict | None:
    ...
    response = await complete(messages=messages, role="editor_quality", temperature=0.3, max_tokens=2048)
