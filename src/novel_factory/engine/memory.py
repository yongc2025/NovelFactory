...
class MemoryManager:
    ...
    async def generate_scene_summary(self, scene_text: str) -> str:
        ...
        summary = await complete(messages=messages, temperature=0.2, max_tokens=200)
    ...
    async def generate_chapter_summary(self, scenes: list[dict]) -> str:
        ...
        summary = await complete(messages=messages, temperature=0.2, max_tokens=400)
    ...
    async def update_character_state(self, character_id: str, scene_summary: str) -> None:
        ...
        with get_connection() as conn:
            ...
            new_state = await complete(messages=messages, temperature=0.3, max_tokens=100)
            conn.execute(
