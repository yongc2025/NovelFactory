...
import { ArrowLeftOutlined, ArrowRightOutlined, RocketOutlined, ReloadOutlined } from '@ant-design/icons-vue'
...
import { PLATFORM_CARDS, GENRE_CARDS, PROTAGONIST_TEMPLATES, HEROINE_TEMPLATES, ROMANCE_MODES } from '@/components/creation/presets'
...
const genreOptions = [
  ...
  { label: '现代', value: '现代', children: ['总裁豪门', '娱乐圈', '职场逆袭', '都市情感', '悬疑推理'] },
  { label: '玄幻', value: '玄幻', children: ['系统流', '重生修仙', '异世大陆', '灵异悬疑', '末日生存', '种田基建'] },
  { label: '科幻', value: '科幻', children: ['星际', '赛博朋克', '时间循环', '末日废土'] },
  { label: '都市', value: '都市', children: ['校园', '职场', '家庭伦理', '情感婚姻'] },
...
function resetAll() {
  ...
  fullStep.value = 0
  Object.assign(quick, {
    gender: null, genreId: null, platformId: null,
    protagonistName: '', protagonistId: null, heroineId: null,
    romanceMode: null, storyBackground: '', styleText: '',
    referenceWorks: '', forbiddenElements: '',
    inspirationVersions: [], inspirationGenerated: false, inspirationLoading: false,
  })
  Object.assign(guided, {
    gender: null, genreText: '', conflictText: '', platformCard: null,
    protagonistId: null, heroineId: null, characterText: '',
    romanceMode: null, styleText: '', generatedPremise: '',
  })
}

// 自动保存（watch 深度监听）
watch([() => ({ ...quick }), () => ({ ...guided }), selectedMode, quickStep, fullStep], () => {
...
const quickCanProceed = computed(() => {
  switch (quickStep.value) {
    case 0: return !!quick.gender && !!quick.genreId
    case 1: return !!quick.platformId
    case 2: return quick.inspirationGenerated && !!quick.inspirationVersions.find(v => v.selected)
    case 3: return true
...
const canProceed = computed(() => {
  switch (fullStep.value) {
    case 0: return !!guided.gender
    case 1: return guided.genreText.trim().length > 0
    case 2: return guided.conflictText.trim().length > 0
    case 3: return !!guided.platformCard
    case 4: return true
    case 5: return true
    case 6: return true
    default: return true
...
function isRomanceStep(stepIdx: number): boolean {
  return guided.gender === 'male' && guided.romanceMode !== 'none' && stepIdx === 5
}

function isStyleStep(stepIdx: number): boolean {
  if (guided.gender === 'male' && guided.romanceMode !== 'none') return stepIdx === 6
  return stepIdx === 5
}
...
async function generateInspiration() {
  ...
  const protagonist = PROTAGONIST_TEMPLATES.find(t => t.id === quick.protagonistId)
  ...
  try {
    const res = await http.post('/generate/inspiration', {
      ...
      protagonist_desc: protagonist ? protagonist.name + '（' + protagonist.desc + '）' : '',
    ...
    quick.inspirationVersions = (data.versions || []).map((v: any, i: number) => ({
      id: v.id || 'v' + (i + 1),
      synopsis: v.synopsis || '',
      titles: v.titles || [],
      selected: i === 0,
    ...
    message.error(e.response?.data?.detail || e.message || '灵感生成失败，请重试')
  } finally {
...
function buildQuickParams(): CreateProjectParams {
  ...
  if (quick.forbiddenElements) {
    params.forbidden_elements = quick.forbiddenElements.split(/[,，]/).map(s => s.trim()).filter(Boolean)
  }
...
function generatePremise(): string {
  ...
  const protagonist = PROTAGONIST_TEMPLATES.find(t => t.id === guided.protagonistId)
  if (protagonist) parts.push(`主角性格：${protagonist.name}（${protagonist.desc}）`)

  if (guided.characterText) parts.push(`角色补充：${guided.characterText}`)

  const heroine = HEROINE_TEMPLATES.find(t => t.id === guided.heroineId)
  if (heroine && guided.gender === 'male' && guided.romanceMode && guided.romanceMode !== 'none') {
  ...
  if (platform) parts.push(`发布场景：${platform.name}，目标${(platform.targetWords / 10000).toFixed(0)}万字/${platform.targetChapters}章`)
...
function buildFullParams(): CreateProjectParams {
  ...
  return {
    ...
    genre_major: (guided.genreText.split('（')[0] || '').slice(0, 20) || undefined,
...
async function handleSubmit() {
  ...
  try {
    const params = selectedMode.value === 'quick'
      ? buildQuickParams()
      : showAdvanced.value
        ? {
  ...
  <AppLayout>
    <div class="project-create">
      ...
      <div class="create-header">
        <Button @click="selectedMode ? (selectedMode = null) : router.push('/')">
      ...
      <div v-else-if="selectedMode === 'quick'" class="quick-mode">
        ...
        <div class="steps-bar">
          <Steps :current="quickStep" size="small" class="custom-steps">
            <Steps.Step v-for="step in quickSteps" :key="step.title" :title="step.title" />
          </Steps>
        </div>
          ...
          <Transition :name="slideDirection === 'left' ? 'slide-left' : 'slide-right'" mode="out-in">
            <div :key="quickStep">
              ...
              <QuickStep4
                ...
                :reference-works="quick.referenceWorks"
                :forbidden-elements="quick.forbiddenElements"
                :selected-inspiration="quick.inspirationVersions.find(v => v.selected) || null"
                :loading="submitting"
                @submit="handleSubmit"
              />
            </div>
      ...
      <div v-else-if="selectedMode === 'full'" class="full-mode">
        ...
        <div v-if="!showAdvanced" class="guided-mode">
          ...
          <div class="steps-bar">
            <Steps :current="fullStep" size="small" class="custom-steps">
              <Steps.Step v-for="step in steps" :key="step.title" :title="step.title" />
            </Steps>
          </div>
        ...
        <div v-else class="advanced-mode">
          <div class="advanced-header">
            <span class="back-link" @click="switchToGuided">← 返回引导模式</span>
          </div>

          <div class="advanced-form">
            <div class="form-section">
              <div class="form-label">灵感描述 <span class="required">*</span></div>
              <a-textarea
            ...
            <div class="form-row">
              <div class="form-section">
                ...
                <a-select v-model:value="advancedForm.platform" style="width: 100%">
                  <a-select-option value="番茄小说">番茄小说</a-select-option>
                  <a-select-option value="起点中文网">起点中文网</a-select-option>
                  <a-select-option value="晋江文学城">晋江文学城</a-select-option>
                  <a-select-option value="小红书">小红书</a-select-option>
                </a-select>
              </div>
              <div class="form-section">
                <div class="form-label">目标篇幅</div>
                <a-select v-model:value="advancedForm.word_count_target" style="width: 100%">
                  <a-select-option :value="10000">短篇（1万字以内）</a-select-option>
                  <a-select-option :value="90000">中篇（10-30万字）</a-select-option>
                  <a-select-option :value="300000">长篇（30-100万字）</a-select-option>
                  <a-select-option :value="1000000">超长篇（100万字以上）</a-select-option>
                </a-select>
              </div>
            </div>
